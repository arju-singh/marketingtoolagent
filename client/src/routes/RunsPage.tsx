  ...
  return (
    <main className="grid-bg min-h-screen">
      <nav className="mx-auto flex max-w-5xl items-center justify-between px-6 py-6">
        <Link to="/" className="text-lg font-bold">Market<span className="gradient-text">Stack</span></Link>
        <div className="flex items-center gap-4">
          <AuthButton />
          <Link to="/app" className="text-sm text-white/50 hover:text-white">Open the tool →</Link>
        </div>
      </nav>
      ...
      <section className="mx-auto max-w-5xl px-6 py-10">
        ...
        {enabled && user && !busy && runs.length === 0 && (
          <p className="mt-4 text-white/55">No runs yet. <Link to="/app" className="text-accent2">Generate your first suite →</Link></p>
        )}
